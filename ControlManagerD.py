...
class Control:
    def __init__(self, id, masters, slaves, ip, port):
        self.id = id
...
class ControlManager(BaseHTTPRequestHandler):   
    def do_POST(self):
        ...
        for key, value in data.items():       
            ControlManager.next_port += 1
            control = Control(key, value['masters'], value['slaves'], self.server.server_address[0], self.next_port)              
